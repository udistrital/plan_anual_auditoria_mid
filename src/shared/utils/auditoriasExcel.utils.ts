...
export async function descargarAuditorias(
  ...
  try {
    ...
    const maxRowIndex = Math.max(
      dataSheetOriginalMaxRows,
      dataSheetStartRowIndex + rows.length - 1,
    );
    ...
    const newError = new Error(
      'Error al incrustar los datos en la plantilla de Excel.',
    );
    newError.stack +=
...
export async function setupValidationDomains(
  ...
  try {
    ...
    await workbook.xlsx.load(inputBuffer);
    console.debug(
      'headersValidacion in descargarAuditorias:',
      headersValidacion,
    );
    ...
    Object.keys(headersValidacion).forEach((header) => {
      ...
      if (domainLength > maxDomainLength) maxDomainLength = domainLength;
    });
    ...
    for (
      let i = domainSheetRowOffset;
      i <= maxDomainLength + domainSheetRowOffset;
      i++
    ) {
      ...
      Object.keys(headersValidacion).forEach((header, index) => {
        ...
        currentCell.value =
          headersValidacion[header][i - domainSheetRowOffset] ?? '';
      });
      currentRow.commit();
    }
    ...
    rowsLengths.forEach((sheetInfo) => {
      ...
      const maxRowIndex =
        rowsLength == null
          ...
          : Math.max(
              dataSheetOriginalMaxRows,
              dataSheetStartRowIndex + rowsLength - 1,
            );

      Object.keys(headersValidacion).forEach((header) => {
        const headerColIdx = HEADERS.indexOf(header) + 1;
        const originColumnLetter = String.fromCharCode(
          64 + validationRanges[header].colIdx,
        );
        ...
        try {
          worksheet
            ...
            .eachCell(
              { includeEmpty: true },
              (cell: any, rowNumber: number) => {
                if (
                  rowNumber >= dataSheetStartRowIndex &&
                  rowNumber <= maxRowIndex
                ) {
                  cell.dataValidation = {
                    type: 'list',
                    allowBlank: true,
          ...
          console.warn(
            `Error applying data validation for header "${header}":`,
            error,
          );
        }
      });
    ...
    const newError = new Error(
      'Error al configurar los dominios de validación en la plantilla de Excel.',
    );
    newError.stack +=
      '\nCaused by: ' + (error instanceof Error ? error.stack : String(error));
    throw newError;
