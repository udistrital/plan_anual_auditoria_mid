...
export class AccionMejoraEstadoService {
  ...
  async getAll(queryParams: any) {
    ...
    const data = await this.auditoriaCrudService.traerDataCrud('accion-mejora-estado', null, queryParams);
  ...
  private async cargarEstados(): Promise<void> {
    ...
    const dominio = await firstValueFrom(
      this.dominiosService.getParametros(TIPO_PARAMETRO.ACCION_ESTADOS),
    );
