...
describe('AuditoriaService', () => {
  ...
  describe('herencia de campos padre', () => {
    it('getByAuditor debe heredar tipo_evaluacion_id y dependencia_id desde padre', async () => {
      ...
      mockDominiosService.getDependencias.mockReturnValue(
        of({ parametros: [] }),
      );

      const res = await service.getByAuditor('personaX', {
        query: 'tipo_evaluacion_id:7',
      });
      ...
      expect(mockCrudService.traerDataCrud).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          query: expect.stringContaining('activo:true'),
        }),
      );
      expect(mockCrudService.traerDataCrud).toHaveBeenCalledWith(
        'auditoria/auditor',
        expect.any(String),
        expect.objectContaining({
          query: expect.stringContaining('auditoria_padre_id__in:p1'),
        }),
      );
      expect(mockCrudService.traerDataCrud).toHaveBeenCalledWith(
        'auditoria/auditor',
      ...
      expect(mockCrudService.traerDataCrud).toHaveBeenCalledWith(
        ...
        null,
        expect.objectContaining({
          query: expect.stringContaining('tipo_evaluacion_id:7'),
        }),
      );
      ...
      const llamóPadre = calls.some(
        ([resource, id, params]) =>
          resource === 'auditoria-padre' &&
          params &&
          params.fields &&
          params.fields.includes('tipo_evaluacion_id') &&
          params.fields.includes('dependencia_id'),
      );
    ...
    it('getByDependencia debe heredar tipo_evaluacion_id y dependencia_id desde padre', async () => {
      ...
      mockDominiosService.getParametros.mockReturnValue(of({ parametros: [] }));
      mockDominiosService.getDependencias.mockReturnValue(
        of({ parametros: [{ Id: 42, Nombre: 'Dep42' }] }),
      );

      const res = await service.getByDependencia(1, 312, {
        query: 'tipo_evaluacion_id:9',
      });
      expect(res.Data).toBeDefined();
      expect(res.Data.length).toBe(1);
      ...
      expect(mockCrudService.traerDataCrud).toHaveBeenCalledWith(
        ...
        null,
        expect.objectContaining({
          query: expect.stringContaining('dependencia_id__in:42'),
        }),
      );
      ...
      const calls = mockCrudService.traerDataCrud.mock.calls;
      const padreCall = calls.find(
        ([resource]) => resource === 'auditoria-padre',
      );
