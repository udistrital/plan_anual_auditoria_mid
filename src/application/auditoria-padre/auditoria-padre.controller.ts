...
export class AuditoriaPadreController {
  ...
  @ApiQuery({ name: 'plan_auditoria_id', required: true, description: 'ID del plan de auditoría (obligatorio).' })
  @ApiQuery({ name: 'orderBy', required: false, description: 'Campo de orden.' })
  @ApiQuery({ name: 'orderDirection', required: false, description: 'Dirección (ASC o DESC).' })
  ...
  async getAuditoriasOrdenadas(@Res() res: any, @Query() queryParams: any) {
    try {
      const data = await this.auditoriaPadreService.getAuditoriasOrdenadas(queryParams);
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID de auditoría padre.' })
  ...
  async delete(@Res() res: any, @Param('id') id: string, @Param('planId') planId: string) {
    try {
      const data = await this.auditoriaPadreService.deleteAuditoriaPadre(id, planId);
      res.status(HttpStatus.OK).json(data);
    } catch (error) {
      res.status(error.status || HttpStatus.INTERNAL_SERVER_ERROR).json({
        Success: false,
