...
export class InformeService {
    ...
    try {
      ...
      const complementos$ = forkJoin({
        ...
        hallazgos: from(
          ...
          catchError((error) => {
            this.logger.warn(
              `No se pudieron obtener hallazgos: ${error.message}`,
            );
            return of([]);
          }),
        ),
      ...
      this.logger.log(
        `Informe ${id} consultado exitosamente con datos complementarios`,
      );
      ...
      this.logger.error(
        `Error al obtener informe completo: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
  ...
  async getAll(queryParams: any) {
    try {
      ...
      this.logger.error(
        `Error al obtener todos los informes: ${error.message}`,
        error.stack,
      );
      throw new HttpException(
